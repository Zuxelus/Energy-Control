...
public class ClientTickHandler {
	...
	public static void render(RenderWorldLastEvent event) {
		...
		info.update(client.world, client.getRenderViewEntity() == null ? client.player : client.getRenderViewEntity(),
			client.gameSettings.thirdPersonView > 0, client.gameSettings.thirdPersonView == 2, event.getPartialTicks());
		...
		Vec3d vector3d = info.getProjectedView();
		...
		for (TileEntity te : holo_panels) {
			...
			TileEntityRendererDispatcher.instance.renderTileEntity(te, -1, matrixStack, buffers);
			matrixStack.pop();
		}
		holo_panels.clear();
	}
