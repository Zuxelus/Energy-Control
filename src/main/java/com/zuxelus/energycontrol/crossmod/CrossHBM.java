...
public class CrossHBM extends CrossModBase {
	...
	public NBTTagCompound getCardData(World world, BlockPos pos) {
		...
		if (te instanceof TileEntityBarrel) {
			NBTTagCompound tag = new NBTTagCompound();
			FluidInfo.addTank("tank", tag, ((TileEntityBarrel) te).tank);
			return tag;
		}
		if (te instanceof TileEntityProxyCombo) {
			NBTTagCompound tag = new NBTTagCompound();
			...
			if (base instanceof TileEntityMachineLargeTurbine) {
				...
				if (values != null) {
					...
					tag.setDouble("consumption", (int) values.get(0));
					tag.setDouble("output", (int) values.get(2));
					tag.setDouble("outputmb", (int) values.get(1));
				}
			...
			if (base instanceof TileEntityITER) {
				...
				tag.setDouble("consumption", reactor.isOn ? output * 10 : 0);
				tag.setDouble("outputmb", reactor.isOn ? output : 0);
			...
			if (base instanceof TileEntityTowerSmall) {
				...
				FluidInfo.addTank("tank", tag, ((TileEntityTowerSmall) base).tanks[0]);
				FluidInfo.addTank("tank2", tag, ((TileEntityTowerSmall) base).tanks[1]);
				return tag;
			}
			if (base instanceof TileEntityChungus) {
