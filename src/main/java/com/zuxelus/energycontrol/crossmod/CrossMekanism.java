...
public class CrossMekanism extends CrossModBase {
	...
	public CompoundNBT getCardData(TileEntity te) {
		...
		if (te instanceof TileEntityBoilerCasing) {
			return boilerTag(((TileEntityBoilerCasing) te).structure);
		}
		if (te instanceof TileEntityBasicLaser) {
	...
	public static CompoundNBT boilerTag(SynchronizedBoilerData block) {
		if (block == null)
			return null;
		...
		CrossMekanism.addTank("tank", tag, block.waterTank);
		CrossMekanism.addTank("tank2", tag, block.steamTank);
		return tag;
	}
	...
	private static String formatTemp(TemperatureUnit unit, double temp) {
		return String.format("%.3f %s", unit.convertFromK(temp, true), tempSymbol(unit));
	}

	private static String tempSymbol(TemperatureUnit unit) {
		switch (unit) {
		case AMBIENT:
			return "+STP";
		case CELSIUS:
