...
public class GuiHoloPanel extends GuiPanelBase { 
	private static final ResourceLocation TEXTURE = new ResourceLocation(EnergyControl.MODID, "textures/gui/gui_holo_panel.png");

	public GuiHoloPanel(ContainerBase container) {
		super(container, "tile.holo_panel.name", TEXTURE);
		ySize = 201;
		...
		if (ItemCardMain.isCard(stack)) {
			...
			if (settingsList != null)
				for (PanelSetting panelSetting : settingsList) {
					addButton(new GuiInfoPanelCheckBox(0, x + 4, guiTop + 28 + hy * y, panelSetting, panel, slot, fontRendererObj));
					y++;
				}
			if (!modified) {
				textboxTitle = new GuiTextField(0, fontRendererObj, 7, 16, 162, 18);
