...
public class GuiScreenColor extends GuiBase {
	...
	public void init() {
		super.init();
		fieldList.clear();
		rText = new GuiTextNumeric(font, guiLeft + 10, guiTop + 18, 26, 12, StringTextComponent.EMPTY, 255);
		rText.setMaxLength(3);
		rText.setValue(Integer.toString((colorText & 0x00FF0000) >> 16));
		//rText.setEnableBackgroundDrawing(false);
		fieldList.add(rText);
		gText = new GuiTextNumeric(font, guiLeft + 46, guiTop + 18, 26, 12, StringTextComponent.EMPTY, 255);
		gText.setMaxLength(3);
		gText.setValue(Integer.toString((colorText & 0x0000FF00) >> 8));
		fieldList.add(gText);
		bText = new GuiTextNumeric(font, guiLeft + 82, guiTop + 18, 26, 12, StringTextComponent.EMPTY, 255);
	...
	protected void drawGuiContainerBackgroundLayer(MatrixStack matrixStack, float partialTicks, int mouseX, int mouseY) {
		...
		minecraft.getTextureManager().bind(PICKER);
