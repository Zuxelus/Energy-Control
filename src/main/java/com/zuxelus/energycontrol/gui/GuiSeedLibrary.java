...
public class GuiSeedLibrary extends GuiContainer {
	...
	public void initGui() {
		super.initGui();
		GuiButtonImage importButton = new GuiButtonImage(0, guiLeft + 132, guiTop + 88, 18, 20, 176, 0, 21, TEXTURE);
		// importButton.tooltip = "Import seeds";
		buttonList.add(importButton);

		GuiButtonImage exportButton = new GuiButtonImage(1, guiLeft + 151, guiTop + 88, 18, 20, 176, 42, 21, TEXTURE);
		// exportButton.tooltip = "Export seeds";
		buttonList.add(exportButton);

		unk_type_button = new GuiButtonImage(2, guiLeft + left + main_width / 8 - 9, guiTop + middle + 20, 18, 20, 176, 84, 21, TEXTURE);
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
		...
		if (tooltip != null && tooltip.length() > 0)
			drawCreativeTabHoveringText(tooltip, mouseX - guiLeft, mouseY - guiTop);
	...
	private String getTooltip(int x, int y) {
		...
		for (Object control : buttonList) {
			if (control instanceof GuiButtonImage) {
				String tooltip = ((GuiButtonImage) control).getActiveTooltip(x, y);
				if (tooltip != null)
					return tooltip;
			}
		}
