...
public class GuiTimer extends GuiContainerBase<ContainerTimer> {
	...
	public void init() {
		...
		addButton(new CompactButton(0, guiLeft + 14, guiTop + 50, 34, 12, "+1", (button) -> { actionPerformed(button); }));
		addButton(new CompactButton(1, guiLeft + 14, guiTop + 64, 34, 12, "+10", (button) -> { actionPerformed(button); }));
		addButton(new CompactButton(2, guiLeft + 50, guiTop + 50, 34, 12, "+100", (button) -> { actionPerformed(button); }));
		addButton(new CompactButton(3, guiLeft + 50, guiTop + 64, 34, 12, "+1000", (button) -> { actionPerformed(button); }));
		addButton(new CompactButton(4, guiLeft + 14, guiTop + 78, 70, 12, "+10000", (button) -> { actionPerformed(button); }));

		addButton(new CompactButton(5, guiLeft + 14, guiTop + 36, 34, 12, "Reset", (button) -> { actionPerformed(button); }));
		addButton(new CompactButton(6, guiLeft + 50, guiTop + 36, 34, 12, "Ticks", (button) -> { actionPerformed(button); }));
		addButton(new CompactButton(7, guiLeft + 14, guiTop + 92, 70, 12,
				timer.getInvertRedstone() ? "No Redstone" : "Redstone", (button) -> { actionPerformed(button); }));
		addButton(new CompactButton(8, guiLeft + 14, guiTop + 106, 70, 12,
				lastIsWorking ? "Stop" : "Start", (button) -> { actionPerformed(button); }));
	...
	private void updateCaptions(boolean isTicks) {
		buttons.get(0).setMessage(isTicks ? "+1" : "+1s");
		buttons.get(1).setMessage(isTicks ? "+10" : "+30s");
		buttons.get(2).setMessage(isTicks ? "+100" : "+1m");
		buttons.get(3).setMessage(isTicks ? "+1000" : "+30m");
		buttons.get(4).setMessage(isTicks ? "+10000" : "+1h");
		buttons.get(6).setMessage(isTicks ? "Ticks" : "Time");
	}

	@Override
	protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
		super.drawGuiContainerBackgroundLayer(partialTicks, mouseX, mouseY);
		textboxTimer.renderButton(mouseX, mouseY, partialTicks);
	}
	...
	public void tick() {
		...
		if (isWorking != lastIsWorking) {
			...
			buttons.get(8).setMessage(isWorking ? "Stop" : "Start");
	...
	protected void actionPerformed(Button button) {
		...
		case 7:
			...
			buttons.get(7).setMessage(!invertRedstone ? "No Redstone" : "Redstone");
			break;
		case 8:
			updateTime(0);
			boolean isWorking = timer.getIsWorking();
