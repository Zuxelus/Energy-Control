		...
		if (isServer) {
			...
			for (int i = 0; i < 6; i++)
				if (reader.hasField("slot" + Integer.toString(i))) {
					ItemStack stack = ItemStack.loadItemStackFromNBT(reader.getTag("slot" + Integer.toString(i)));
					result.add(new PanelString(String.format("msg.ec.InfoPanelSlot%d", i + 1), StringUtils.getItemName(stack) + " x" + Integer.toString(stack.stackSize), showLabels));
				}
		}
		return result;
	}
