...
public class ItemKitMain extends Item implements IItemKit {
	...
	public static ItemStack getKitById(int id) {
		return KITS.containsKey(id) ? new ItemStack(ModItems.itemKit, 1, id) : null;
	}
	...
	public static void onRightClickBlock(PlayerInteractEvent.RightClickBlock event) {
		...
		if (!player.worldObj.isRemote && stack != null) {
			...
			if (item instanceof IItemKit) {
				ItemStack card = ((IItemKit) item).getSensorCard(stack, player, player.worldObj, event.getPos(), event.getFace());
