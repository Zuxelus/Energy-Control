	...
	public void playAlarm(double x, double y, double z, String name, float range) {
		...
		if (person != null) {
			...
			if (volume > 0) {
				if (volume < 0.3)
					volume = 0.3;
				sound = new SimpleSoundInstance(SoundEvent.createVariableRangeEvent(new ResourceLocation(name)), SoundSource.MASTER, (float) volume, 1.0F, SoundInstance.createUnseededRandom(), (float) person.x, (float) person.y, (float) person.z);
